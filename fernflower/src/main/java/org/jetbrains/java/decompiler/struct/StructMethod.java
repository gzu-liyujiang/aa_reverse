...
import org.jetbrains.java.decompiler.struct.attr.StructParameterTableAttribute;
...
public class StructMethod extends StructMember {
  ...
  private StructParameterTableAttribute parameterTable;
  ...
  protected StructGeneralAttribute readAttribute(DataInputFullStream in, ConstantPool pool, String name) throws IOException {
    if (StructGeneralAttribute.ATTRIBUTE_CODE.equals(name)) {
     ...
     else if(StructGeneralAttribute.ATTRIBUTE_PARAMETER_TABLE.equals(name)){//from AIDE
         StructParameterTableAttribute attribute =new StructParameterTableAttribute();
          int length = in.readInt();
       
          attribute.initContent(in,pool);
        
          parameterTable =attribute;
  ...
  public String[] getParameters(){//from AIDE
      return parameterTable!=null?parameterTable.getEntries():null;
  }
