...
public class VarExprent extends Exprent {
  ...
  private void appendDefinitionType(TextBuffer buffer) {
    ...
    if(isInnerAnonymousClassType(type))
        buffer.append(ExprProcessor.getCastTypeName(getSuperClassType(type)));
    else
  ...
  public static VarType getSuperClassType(VarType classType){
      ClassNode currentClass=DecompilerContext.getClassProcessor().getMapRootClasses().get(classType.value);
      String superClass=currentClass.getWrapper().getClassStruct().superClass.getString();  
      return new VarType(superClass,true);
  }
  
  public static boolean isInnerAnonymousClassType(VarType type){
      int p=type.value.lastIndexOf('$');
      if(p==-1)return false;
      
      byte class_name_start_char=type.value.getBytes()[p+1];
      
      if(class_name_start_char>='0'
        &&class_name_start_char<='9')
          return true;
      return false;
  }
