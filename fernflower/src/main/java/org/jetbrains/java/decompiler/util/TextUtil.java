...
public class TextUtil {
  ...
  private static boolean isClassInnerAnonymousClass(String className){
      int p=className.lastIndexOf('$');
      if(p==-1) return false;
      byte ch=className.getBytes()[p+1];
      return ch>='0'&&ch<='9';
  }
  ...
  public static void writeQualifiedSuper(TextBuffer buf, String qualifier) {
    ...
    if (!isClassInnerAnonymousClass(qualifier)&&!qualifier.equals(classNode.classStruct.qualifiedName)) {
