...
public class VarDefinitionHelper {
  ...
  public VarDefinitionHelper(Statement root, StructMethod mt, VarProcessor varproc) {
    ...
    String[] paramsTable=mt.getParameters();
    ...
    for (int i = 0; i < paramcount; i++) {
      implDefVars.add(varindex);
        String param=null;
        if(paramsTable!=null&&paramsTable.length>0&&paramsTable.length>=i)
            param=paramsTable[i!=0?i-1:0];
        varproc.setVarName(new VarVersionPair(varindex, 0),param!=null?param:"p"+i);
    ...
    while (!stack.isEmpty()) {
      ...
      if (lstVars != null) {
        for (VarExprent var : lstVars) {
            ...
            implDefVars.add(index);
            
            String param=null;
            if(paramsTable!=null&&paramsTable.length>0&&paramsTable.length>=index)
                param=paramsTable[index!=0?index-1:0];
            varproc.setVarName(new VarVersionPair(var),param!=null?param:"p"+index);
  ...
  public void setVarDefinitions() {
    ...
    for (Entry<Integer, Statement> en : mapVarDefStatements.entrySet()) {
      ...
      varproc.setVarName(pair, vc.getFreeName(varproc.getVarType(pair),index));
