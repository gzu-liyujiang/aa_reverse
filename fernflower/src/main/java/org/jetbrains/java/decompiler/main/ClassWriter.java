...
//import org.jetbrains.java.decompiler.main.collectors.SyntheticCollector;
...
public class ClassWriter {
    ...
    /*
    private static void syntheticProcessors(ClassNode node,SyntheticCollector collector) {
        ClassWrapper wrapper = node.getWrapper();
        StructClass cl = wrapper.getClassStruct();
        
        for (StructField fd : cl.getFields()) {
            if(fd.isSynthetic())
                collector.addSyntheticField(cl,fd);
        }
        
        for (StructMethod mt : cl.getMethods()) {
            if(mt.isSynthetic())
                collector.addSyntheticMethod(cl,mt);
        }
    }*/
    ...
    public void classToJava(ClassNode node, TextBuffer buffer, int indent, BytecodeMappingTracer tracer) {
        ...
        try {
            ...
            //syntheticProcessors(node,DecompilerContext.getSyntheticCollector());
            ...
            for (ClassNode inner : node.nested) {
                if (inner.type == ClassNode.CLASS_MEMBER) {
                    ...
                    if(isInnerAnonymousClass(inner.classStruct.qualifiedName))
                        continue;// inner anonymous class
    ...
    public void fieldToJava(ClassWrapper wrapper, StructClass cl, StructField fd, TextBuffer buffer, int indent, BytecodeMappingTracer tracer) {
    ...
    public boolean methodToJava(ClassNode node, StructMethod mt, TextBuffer buffer, int indent, BytecodeMappingTracer tracer) {
        ClassWrapper wrapper = node.getWrapper();
        StructClass cl = wrapper.getClassStruct();
        MethodWrapper methodWrapper = wrapper.getMethodWrapper(mt.getName(), mt.getDescriptor());
            ...
            if (DecompilerContext.getOption(IFernflowerPreferences.DECOMPILE_GENERIC_SIGNATURES)) {
                ...
                if (attr != null) {
                    ...
                    if (descriptor != null) {
                        ...
                        if (sigFields != null) {
                            actualParams = Stream.from(sigFields)
                                .filter(new Predicate<VarVersionPair>(){
                                    @Override
                                    public boolean test(VarVersionPair t){
                                        return t==null;
                                    }
                                })
                                .count();
    ...
    public static boolean isInnerAnonymousClass(String className){
        int p;
        if((p=className.lastIndexOf('$'))==-1)
            return false;
        int classNameStartChar=className.charAt(p+1);
        if(classNameStartChar>='0'&&classNameStartChar<='9')
            return true;
        return false;
    }
