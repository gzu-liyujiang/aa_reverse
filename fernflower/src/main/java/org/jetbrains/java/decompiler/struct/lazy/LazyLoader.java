...
public class LazyLoader {
  ...
  public ConstantPool loadPool(String classname) {
    try {
      Link link=getClassLink(classname);
      DataInputFullStream in =link!=null? getClassStream(link):null;
      if (in != null) {
        in.discard(8);
  ...
  public DataInputFullStream getClassStream(String tag) throws IOException {
    byte[] bytes = provider.getBytecode(tag);
  ...
  public static class Link {
    ...
    public Link(int type, String tag) {
      this.type = type;
      this.tag=tag;
    }
  }
