...
//import jadx.annotation.TestOnly;
...
public class ClassNode extends LineAttrNode implements ILoadable, IDexNode {
	...
	public boolean isAnonymousClass() {
		return clsInfo.isInner()
            && Character.isDigit(clsInfo.getAlias().getShortName().charAt(0));
	}
    ...
    public MethodNode getAnonymousClassConstructor() {
        for (MethodNode mth : methods) {
    ...
    public int getSyntheticFieldCount(){
        ...
        return synthetic_field_count;
    }
    
    public int getExternInstanceReferencedCount(){
        int extern_instance_count=0;
        {
            for(FieldNode f : fields)
                if(f.getName().startsWith("this$"))
                    ++extern_instance_count;
        }
        return extern_instance_count;
    }

    public int getExternFinalValReferencedCount(){
        int extern_final_var_count=0;
        {
            for(FieldNode f : fields)
                if(f.getName().startsWith("val$"))
                    ++extern_final_var_count;
        }
        return extern_final_var_count;
    }
