...
public class InsnGen {
    ...
    public void try_convert_type(CodeWriter code,ArgType base_type, ArgType test_type) throws CodegenException {
        if(!base_type.equals(test_type)){
            code.add('(');
            useType(code,base_type);
            code.add(')');
        }
	}
	...
	private void instanceField(CodeWriter code, FieldInfo field, InsnArg arg) throws CodegenException {
        ...
        if(arg.isRegister()){         
            ...
            if(!arg.getType().equals(field.getDeclClass().getType())){
                convert_type=true;              
            }
		...
		try {
			...
			if (flag == Flags.BODY_ONLY || flag == Flags.BODY_ONLY_NOWRAP) {
                ...
                if(insn.getType()==InsnType.INVOKE){
                    ...
                    if(ret_type!=null){
                        try_convert_type(code,ret_type,callMth.getReturnType());
                    }
                }
	...
	private void makeInsnBody(CodeWriter code, InsnNode insn, Set<Flags> state) throws CodegenException {
		switch (insn.getType()) {
			...
			case MOVE:
                try_convert_type(code,insn.getResult().getType(),insn.getArg(0).getType());
			...
			case IPUT: {
				...
				code.add(" = ");
                try_convert_type(code,fieldInfo.getType(),insn.getArg(0).getType());
				addArg(code, insn.getArg(0), false);
				break;
			}
			...
			case SPUT:
				...
				code.add(" = ");
                try_convert_type(code,field.getType(),insn.getArg(0).getType());
				addArg(code, insn.getArg(0), false);
				break;
	...
	private void makeConstructor(ConstructorInsn insn, CodeWriter code)
		...
		if (cls != null &&cls.isAnonymousClass()/* cls.contains(AFlag.ANONYMOUS_CLASS) */&& !fallback) {
        ...
        int startArgNum=0;
        if(cls!=null){
            ClassInfo c_info=cls.getClassInfo();
            AccessInfo c_flag= cls.getAccessFlags();
            if(/*c_info!=null&&c_flag!=null&&*/c_info.isInner()&&!c_flag.isStatic())
                startArgNum=1;
        }
        //cls.getClassInfo().isInner()&&!cls.getAccessFlags().isStatic()?1:0;
		generateMethodArguments(code, insn,startArgNum, callMth,callMth!=null?callMth.getMethodInfo():null,startArgNum);
	}
	...
	private void inlineAnonymousConstr(CodeWriter code, ClassNode cls, ConstructorInsn insn) throws CodegenException {
        ...
        int arg_off=cls.getSyntheticFieldCount()
            +cls.getExternInstanceReferencedCount()
            +cls.getExternFinalValReferencedCount();
		generateMethodArguments(
          code, 
          insn, 
          arg_off,
          callMth,
          callMth.getMethodInfo(),
          arg_off);
		code.add(' ');
		new ClassGen(cls, mgen.getClassGen().getParentGen()).addClassBody(code);
	}
    
    private void addWrapArg(InsnWrapArg arg,CodeWriter code,Flags flag,ArgType convert_type) throws CodegenException{
        boolean type_convert=false;
        if(convert_type!=null&&!arg.getType().equals(convert_type)){
            type_convert=true;
            code.add("((");
            useType(code,convert_type);
            code.add(')');
        }
        //InsnNode insn=arg.getWrapInsn();
      
        makeInsn(arg.getWrapInsn(), code, flag);
        if(type_convert)
            code.add(')');    
    }
	...
	private void makeInvoke(InvokeNode insn, CodeWriter code) throws CodegenException {
		...
		switch (type) {
			...
			case INTERFACE:
                ...
                callCls=callMth.getDeclClass();
                
                if(arg.isInsnWrap()){
                    addWrapArg((InsnWrapArg)arg,code,Flags.BODY_ONLY,callCls.getType());
                    code.add('.');
                }
    ...
    @Nullable MethodNode callMth,MethodInfo call_info,int type_off) throws CodegenException {
        int k = startArgNum;
        if (callMth != null && callMth.contains(AFlag.SKIP_FIRST_ARG)) {
            k++;
        }
        int argsCount = insn.getArgsCount();
        List<ArgType> types=call_info!=null?call_info.getArgumentsTypes():null;
        code.add('(');      
        ...
        if (k < argsCount) {
            ...
            for (int i = k; i < argsCount; i++) {
                ...
                if(arg.isInsnWrap()){
                    addWrapArg((InsnWrapArg)arg, code, Flags.BODY_ONLY_NOWRAP,types!=null?types.get(i-startArgNum+type_off):null);
                }
                else
                    addArg(code, arg, false);
	...
	void generateMethodArguments(CodeWriter code, InsnNode insn, int startArgNum,
		...
		generateMethodArguments(code,insn,startArgNum,callMth,callMth!=null?callMth.getMethodInfo():null,0);
	}
